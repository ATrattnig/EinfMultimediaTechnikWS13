...
public class HistoGrammViewer extends ApplicationFrame {
    ...
    private IntervalXYDataset createDataset() throws Exception {
        ...
        File picture      = new File("akropolis_bad.png");
        ...
        for (int w = 0; w < (img.getWidth()); w++)
			for (int h = 0; h < (img.getHeight()); h++)
			{
				...
				int grayTrans = 255 * (gray - Gmin) / (Gmax - Gmin);
				if (grayTrans < 0)
					grayTrans = 0;
				if (grayTrans > 255)
					grayTrans = 255;
				//System.out.println(gray + " " + grayTrans);
				Color newColor = new Color(grayTrans, grayTrans, grayTrans);
